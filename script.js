  ...
  if (summary && description && location && startTime && endTime) {
    ...
    try {
      const response = await fetch(
        "https://v1.nocodeapi.com/ritam21/calendar/iAwXQDPkCuAvhATc/event", 
        requestOptions
      );
...
async function fetchEvents() {
  ...
  try {
    const response = await fetch(
      "https://v1.nocodeapi.com/ritam21/calendar/iAwXQDPkCuAvhATc/listEvents",
      requestOptions
    );
...
async function saveEdit(id) {
  ...
  if (summary && description && location && startTime && endTime) {
    ...
    try {
      const response = await fetch(
        "https://v1.nocodeapi.com/ritam21/calendar/iAwXQDPkCuAvhATc/event?eventId=" +
          eventIdentity,
        requestOptions
      );
...
async function deleteItem(id) {
  ...
  try {
    const response = await fetch(
      "https://v1.nocodeapi.com/ritam21/calendar/iAwXQDPkCuAvhATc/event?eventId=" + id, 
      requestOptions
    );
    const result = await response.text();
    console.log(result);
